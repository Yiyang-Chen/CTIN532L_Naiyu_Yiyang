...
public class MusicMgr : SingletonManager<MusicMgr>
{
    ...
    */
    ...
    AudioMixer _mainMixer = ResourceMgr.GetInstance().Load<AudioMixer>("_Music/Mixers/MainMixer");
    public MusicMgr()
    {
    ...
    public void PlayBkMusic(string name)
    {
        if(bkMusic == null)
        {
            ...
            obj.name = "BKMusic";
            MonoManager.GetInstance().DontDestoryOnLoad(obj);
            bkMusic = obj.AddComponent<AudioSource>();
        }

        ResourceMgr.GetInstance().LoadAsyn<AudioClip>("_Music/BackGround/" + name, (AudioClip) =>
        {
    ...
    public void CrossFading(string name)
    {
        if (bkMusic == null)
        {
            PlayBkMusic(name);
            bkMusic.volume = 0;
            MonoManager.GetInstance().StartCoroutine(StartFade(bkMusic,5f, bkValue,false));
        }
        else
        {
            ...
            newBk.volume = 0;
            MonoManager.GetInstance().StartCoroutine(StartFade(oldBk, 2f, 0,true));
            MonoManager.GetInstance().StartCoroutine(StartFade(newBk, 2f, bkValue,false));
    ...
    public void PlaySound(string name, bool isloop, UnityAction<AudioSource> callBack = null)
    {
        if (soundObj == null)
        {
            ...
            soundObj.name = "Sound";
            MonoManager.GetInstance().DontDestoryOnLoad(soundObj);

        }

        ResourceMgr.GetInstance().LoadAsyn<AudioClip>("_Music/SoundEffect/" + name, (clip) =>
        {
    ...
    public IEnumerator StartFade(AudioSource audioSource, float duration, float targetVolume, bool needDestory)
    {
        ...
        if(needDestory) MonoManager.GetInstance().Destory(audioSource);
        yield break;
    }
}
